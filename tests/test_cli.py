...
class TestCliAnonymize(unittest.TestCase):
    ...
    def test_scan_reports_without_creating_output(self):
        buf = io.StringIO()
        with redirect_stdout(buf):
            exit_code = cli.main([
                "scan", "-i", self.in_path, "--salt-file", self.salt_path,
            ])
        self.assertEqual(exit_code, cli.EXIT_OK)
        report = json.loads(buf.getvalue())
        self.assertEqual(report["input"], self.in_path)
        self.assertGreater(report["replacements"], 0)

    def test_scan_csv_and_json_formats(self):
        for fmt in ("csv", "json"):
            buf = io.StringIO()
            with redirect_stdout(buf):
                code = cli.main(["scan", "-i", self.in_path,
                                 "--salt-file", self.salt_path, "--format", fmt])
            self.assertEqual(code, cli.EXIT_OK)
            if fmt == "json":
                self.assertEqual(json.loads(buf.getvalue())[0]["input"], self.in_path)
            else:
                self.assertIn("stats_by_type", buf.getvalue())

    def test_batch_dry_run_does_not_write_outputs(self):
        input_dir = os.path.join(self.tmpdir.name, "dry-in")
        output_dir = os.path.join(self.tmpdir.name, "dry-out")
        os.makedirs(input_dir)
        with open(os.path.join(input_dir, "one.log"), "w", encoding="utf-8") as handle:
            handle.write("user=jdoe\n")
        code = cli.main(["batch", "--input-dir", input_dir,
                         "--output-dir", output_dir, "--dry-run",
                         "--salt-file", self.salt_path])
        self.assertEqual(code, cli.EXIT_OK)
        self.assertFalse(os.path.exists(output_dir))

    def test_verify_result_checks_existing_output_without_rewriting(self):
        output = os.path.join(self.tmpdir.name, "result.log")
        with open(output, "w", encoding="utf-8") as handle:
            handle.write("user=jdoe ip=192.168.1.10\n")
        with open(output, "rb") as handle:
            before = handle.read()
        buf = io.StringIO()
        with redirect_stdout(buf):
            code = cli.main(["verify-result", output, "--salt-file", self.salt_path])
        self.assertEqual(code, cli.EXIT_OK)
        self.assertFalse(json.loads(buf.getvalue())["safe"])
        with open(output, "rb") as handle:
            self.assertEqual(handle.read(), before)

    def test_batch_progress_events_include_eta_and_rate(self):
        input_dir = os.path.join(self.tmpdir.name, "progress-in")
        output_dir = os.path.join(self.tmpdir.name, "progress-out")
        os.makedirs(input_dir)
        with open(os.path.join(input_dir, "one.log"), "w", encoding="utf-8") as handle:
            handle.write("user=jdoe\n")
        err = io.StringIO()
        with redirect_stderr(err):
            code = cli.main(["batch", "--input-dir", input_dir, "--output-dir", output_dir,
                             "--salt-file", self.salt_path, "--progress-events"])
        self.assertEqual(code, cli.EXIT_OK)
        events = [json.loads(line) for line in err.getvalue().splitlines()
                  if line.startswith("{")]
        self.assertTrue(events)
        self.assertIn("rate_files_per_second", events[0])
        self.assertIn("eta_seconds", events[0])
...
class TestCliValidateConfig(unittest.TestCase):
    ...
    def test_custom_pattern_validation_uses_clean_messages(self):
        path = os.path.join(self.tmpdir.name, "cfg.json")
        payload = {
            "org_name": "acme",
            "custom_patterns": {
                "bad": {"pattern": "(invalid", "test_string": "example"},
                "mismatch": {"pattern": r"\d+", "test_string": "example", "strategy": "full"},
            },
        }
        with open(path, "w", encoding="utf-8") as f:
            json.dump(payload, f)
        ...
        self.assertEqual(exit_code, cli.EXIT_ERROR)
        text = buf.getvalue()
        self.assertIn("custom_patterns", text)
