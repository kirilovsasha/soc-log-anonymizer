...
class TestRegexAndRuleAudit(unittest.TestCase):
    def test_validate_regex_pattern_accepts_valid_input(self):
        ok, message = validate_regex_pattern(r"\d+", "123")
        self.assertTrue(ok)
        self.assertIn("valid", message.lower())

    def test_validate_regex_pattern_rejects_invalid_regex(self):
        ok, message = validate_regex_pattern("(", "x")
        self.assertFalse(ok)
        self.assertIn("invalid", message.lower())

    def test_summarize_active_rules_and_conflicts(self):
        rules = summarize_active_rules(
            [
                {"name": "email", "pattern": r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}", "test_string": "user@example.com"},
                {"name": "email", "pattern": r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}", "test_string": "user@example.com"},
            ],
            [{"name": "ctx", "pattern": r"\badmin\b", "test_string": "admin"}],
        )
        self.assertEqual(len(rules), 3)
        self.assertTrue(any(rule["name"] == "email" for rule in rules))
    ...
    def test_changed_line_numbers_reports_both_sides(self):
        left, right = changed_line_numbers("same\nold\n", "same\nnew\n")
        self.assertEqual(left, [2])
        self.assertEqual(right, [2])

    def test_truncate_display_text_keeps_both_ends(self):
        displayed, truncated = truncate_display_text("0123456789", 6)
        self.assertTrue(truncated)
        self.assertIn("0123", displayed)
        self.assertIn("89", displayed)
